  ...
  <b-container fluid class="backg">
    ...
    >
      ...
      <b-form @submit="onSubmit">
        <b-container>
          <h3>{{ $t("speciality.title1") }}</h3>
          <hr />
          <h5>{{ $t("speciality.title2") }}</h5>
          <b-row class="kov-margin">
            <b-col>
              <b-form-group :label="$t('speciality.speciality')" >
                <!--b-form-select
				...
				<multiselect style="border: 1px solid rgb(71, 174, 216);max-height: 34px !important;font-size:14px;"  class="mb-3 select" v-model="prespeciality" :options="specialities"    :multiple="true" :close-on-select="false" :clear-on-select="false" :preserve-search="true"  label="name" track-by="id" :preselect-first="false">
    <template class="mb-3 select" slot="selection" slot-scope="{ values, search, isOpen }"><span class="multiselect__single" v-if="values.length=='0'" >All</span><span class="multiselect__single" v-else="specialities.length &amp;&amp; !isOpen">{{ values.length }} selected</span></template>
  </multiselect>
              </b-form-group>
            </b-col>
...
<div v-if="address!=''"><vue-google-autocomplete 
  ...
  </b-col><!--b-col v-if="showlocation==''"><button @click="detectLocation"><img src="../assets/detect.png" height="20px" /></button></b-col-->
            <b-col>
              <b-form-group :label="$t('speciality.language')">
...
<strong v-if="languagepre.length==0" style="font-size:16px;color:gray;font-weight:500;margin-bottom:20% !important;"  align="right" >All</strong>
 ...
 <div style="position: relative;align:left;" >
      <ul align="left" class="selectdropdown" style="border: 1px solid rgb(71, 174, 216);
...
export default {
  ...
  created() {
    ...
    if (self.specialityId) {
      self.form.specialityId.push(self.specialityId);
      self.Updatecategory(self.specialityId);
    }
    this.isLoading = true;
	self.getLanguages();
    ...
    axios
      ...
      .then(function(response) {
        ...
        console.log(response.data.specialitiesdata);
        self.specialities = response.data.specialitiesdata;
   ...
   getAddressData: function (addressData, placeResultData, id) {
    ...
    onSubmit(evt) {
      ...
      this.specialities.forEach(element => {
        if (element.id == this.form.specialityId) {
         
          console.log("working", element.id);
    ...
    self.form.city = self.address.locality;
     ...
     self.prespeciality.forEach(ele1 => {
          
         
          self.form.specialityId.push(ele1.id)
    ...
    Updatecategory(item) {
      ...
      this.categories.forEach(element => {
        if (element.id === item.category_id) {
          console.log("working", element.id, "cat");
          this.form.categoryId = element.id;
        }
      });
    },
    updateSpeciality() {
      ...
      if (this.form.categoryId != "") {
        axios
          .get("/specialities/" + this.form.categoryId)
          .then(function(response) {
