      ...
      <b-col cols="12">
        <b-row>
          <b-col cols lg="8" sm="12" md="12" >
                ...
                <h5 class="text-left"  style="margin-bottom: 1rem !important;font-size:14px !important;margin-top:10px; margin-left:10px !important;color:#666161;">{{ $t('search.search')}} : {{ $t('online.online-doctors')}} {{ $t("search.near-by") }} {{searchcity}}, {{searchcountry}}</h5>
...
<b-row>
 ...
 <b-col class="text-center" lg="4" sm="6" md="6" xs="6">
          ...
          <router-link :to="`/${$i18n.locale}/service/${item.id}`" class="route">
            <b-row class="container">
              <h4>
                <router-link  :to="`/${$i18n.locale}/providers/${item.id}/${item.branch_name}`.replace(/\s/g, '-')" class="route">{{item.branch_name}}</router-link>
             <b-img src="/static/img/online.jpeg" style="width:12px;height:12px;"></b-img> </h4>
            </b-row>
 ...
 <b-col md="12" lg="4" sm="12" xs="12" v-if="item.category_name !== 'Pharmacy'">
          <b-row v-if="item.slots" class="hidden-scrollbar">
            ...
            <b-container class="bv-example-row">
              ...
              <div class="slots inner text-center display">
                 <!--router-link  :to="`/${$i18n.locale}/providers/${item.id}/${item.branch_name}`.replace(/\s/g, '-')" class="route"-->
                <div v-if="item.slots!=''">
                  <div v-for="slot in item.slots" :key="slot.slot" class="display">
                    <div v-if="slot.available==1" class="display">
                      <div class="display">
                        <b-button size="sm"   class="slotsanimation" @click="withDateSlot(slot.id,slot.start_time,slot.date,item.id,item.branch_name)" >{{slot.start_time}}</b-button>
                        <!-- {{new_slots}} -->
                      </div>
                    </div>
...
<div style="color:#00a4bd;margin-left:9px;" align="left">Showing result: 1 to {{responses.length}} </div>
              ...
              <div v-else>
                <h5 style="color:grey;text-align:center;font-size:14px;">{{ $t('online.not-found')}}</h5>
              </div>
  ...
  methods: {
    ...
    callack(id) {
     ...
     //   let lat = position.coords.latitude;
      //  let long = position.coords.longitude;
        ...
        axios.post("/available-doctors", dataToSend).then(function(response) {
    ...
    //  navigator.geolocation.getCurrentPosition(function(position) {
     ...
     //   let long = position.coords.longitude;
        ...
        axios
          .post("/available-doctors", dataToSend)
          .then(function(response) {
            //console.log("services-revised-date", response);
            self.setBranches(response.data.branches);
            let array = response.data.branches;
