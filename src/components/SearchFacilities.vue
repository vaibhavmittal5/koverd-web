...
<b-col align="left" style="margin-left:10px !important;"> <img src="../assets/filter.png" height="23px" /> <strong style="font-size:14px;color:gray;font-weight:400;text-align:left;" > {{ $t("search.filters") }}</strong>
  <b-dropdown size="sm" variant="outline-info" :text="$t('search.categories')" class="m-md-2" >
    <b-dropdown-item @click="callack('1')"><strong style="font-size:14px;color:#666161;font-weight:500;" >{{$t("search.medical-care")}}</strong></b-dropdown-item>
    <b-dropdown-item @click="callack('2')"><strong style="font-size:14px;color:#666161;font-weight:500;" >{{ $t("search.pharmacy")}}</strong></b-dropdown-item>
    <b-dropdown-item @click="callack('3')"><strong style="font-size:14px;color:#666161;font-weight:500;" >{{$t("search.fitness")}}</strong></b-dropdown-item>
    <b-dropdown-item @click="callack('4')"><strong style="font-size:14px;color:#666161;font-weight:500;" >{{$t("search.health-wellbeing")}}</strong></b-dropdown-item>
    <b-dropdown-item @click="callack('5')"><strong style="font-size:14px;color:#666161;font-weight:500;" >{{$t("search.senior-care")}}</strong></b-dropdown-item>
    <b-dropdown-item @click="callack('6')"><strong style="font-size:14px;color:#666161;font-weight:500;" >{{$t("search.alternative-medicine")}}</strong></b-dropdown-item>
    <b-dropdown-item @click="callack('')"><strong style="font-size:14px;color:#666161;font-weight:500;" >{{ $t('search.near-by')}}</strong></b-dropdown-item>
  </b-dropdown>
 <b-dropdown size="sm"  variant="outline-info" :text="$t('search.specialities')" class="m-md-2" >
...
<option disabled selected value="">Specialities</option>
 <option  v-for="item in specialities"
      ...
      <!---------filter--ends--------------> 
                <h5 class="text-left" style="margin-bottom: 1rem !important;font-size:14px !important;margin-top:10px; margin-left:10px !important;color:#666161;">{{ $t('search.search')}} : {{searchcategory}}  {{ $t("search.near-by") }} {{searchcity}}, {{searchcountry}}</h5>
             
                <!--div style="display:inline-flex;align-items:right;background-color:grey !important;">
<b-row>
          <b-col cols lg="12" sm="12" md="12" >
                  <b-button @click="datePre()" variant="outline-primary" style="border-color: grey; !important">
                ...
                >{{current_date}}</h5>
                  ...
                  <b-button @click="datePost()" variant="outline-primary" style="border-color: grey; !important">
...
<div v-if="this.checkBranches!=''"  style="display:inline-flex;align-items:right;"><b-button @click="removecompareData()" class="compare">
            ...
            <!--div class="hide1filter" style="background-color:#fcfaf5;text-align:left; margin:5px;">
              <b-row>
                  ...
                  <p v-if="show_filters" style="margin:5px;">
                    <b-card>
                      <p class="text-left">
                        ...
                        >
                         <b-form-radio value="Clinic" @change="callack('Clinic')">{{ $t('facilities.Clinic')}}</b-form-radio>
                          <b-form-radio value="Hospital" @change="callack('Hospital')">{{ $t('facilities.Hospital')}}</b-form-radio>
                          <b-form-radio value="Pharmacy" @change="callack('Pharmacy')">{{ $t('facilities.Pharmacy')}}</b-form-radio>
                          <b-form-radio
                            value="Wellness Center"
                            @change="callack('Wellness Center')"
                          >{{ $t('facilities.Wellness Center')}}</b-form-radio>
                          <b-form-radio
                            value="Nursing"
                            @change="callack('Nursing')"
                          >{{ $t('facilities.Nurshing')}}</b-form-radio>
                          <b-form-radio
                            value="Individual"
                            @change="callack('Individual')"
                          >{{ $t('facilities.Individual')}}</b-form-radio>
...
<!--b-button @click="createPDF"  style="border-color: white;background-color:grey;width:100px!important;height:30px !important;align-items:center;display:none !important;">Download as PDF</b-button-->
</b-modal>
...
<b-row>
 ...
 <b-col class="text-center" lg="4" sm="6" md="6" xs="6">
          <b-row>
            <b-col lg="6" sm="6">
              <h6 style="text-align:center;font-weight:500;color:#666161 !important;font-size:12px;display:inline-flex;">{{item.category_name}}</h6>
            </b-col>
            <b-col lg="6" sm="6" class="distance" style="text-align:center;font-weight:500;color:#666161 !important;">{{item.distance.toString().slice(0,4) }} KM</b-col>
          </b-row>
          <router-link  :to="`/${$i18n.locale}/providers/${item.id}/${item.branch_name}`.replace(/\s/g, '-')" class="route">
            <b-row class="container">
              <h4>
                <router-link :to="`/${$i18n.locale}/providers/${item.id}/${item.branch_name}`.replace(/\s/g, '-')" class="route"  style="text-align:center;font-weight:500;color:black !important;font-size:15px;">{{item.branch_name}}</router-link>
              </h4>
            </b-row>
...
<!--------------------end----------------->
 ...
 <!--b-row class="justify-content-center" style="margin-top: 12px" v-if="item.removeFavourites">
      ...
      <b-row align="center">
                    ...
                    <b-col align="right"  v-if="item.category_name !== 'Pharmacy'">
...
<div style="color:#00a4bd;margin-left:9px;" align="left">Showing result: 1 to {{responses.length}} </div>
              ...
              <div v-else>
                <h5 style="color:grey;text-align:center;font-size:14px;margin-top:15px;">{{ $t('search.not-found')}}</h5>
              </div>
  ...
  else
  {
    ...
    callack(id) {
     ...
     //   let long = position.coords.longitude;
        ...
        let dataToSend = {
          category: self.categoryId_filter,
        ...
        self.searchcategory = dataToSend.category;

        axios.post("/services-facilities-new", dataToSend).then(function(response) {
   ...
   //   navigator.geolocation.getCurrentPosition(function(position) {
    ...
    //    let long = position.coords.longitude;
        ...
        axios
          .post("/services-facilities-new", dataToSend)
...
withDateSlot(slot_id,slot_name,date,branch_id,branch_name)
{
 ...
 mymodal(val) {
  ...
  currentDate.setHours( currentDate.getHours() + 2 );
    ...
    this.categoryId_filter = this.category;
    this.getServices();
