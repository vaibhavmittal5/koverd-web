...
export default {
  ...
  created() {
    var self = this;
	self.vc_id=self.$route.params.videoconferencingID;
            ...
            axios
              ...
              .then(function(response) {
                ...
                self.responses.forEach(ele => {
                  if (ele.video_consultation === 1 && ele.date == self.today && ele.vc_id === self.vc_id && ele.vc_start_time !='' && ele.vc_end_time=='Null') {
    ...
    getAppointmnets() {
      ...
      axios
        ...
        .then(function(response) {
          ...
          console.log(self.responses, self.today);
          self.responses.forEach(ele => {
            if (ele.video_consultation === 1 && ele.date == self.today && ele.vc_id !='' && ele.vc_id === self.vc_id && ele.vc_start_time !='' && ele.vc_end_time=='Null') {
              self.appointmentID = ele.id;
              console.log(ele, self.appointmentID);
             self.$router.push(
                      `/en/confo/${ele.vc_id}/participant/Patient-${ele.users.data.first_name}`
