  ...
  <b-container fluid class="backg">
    ...
    <b-col cols="12" v-if="!isLoading">
      ...
      <b-form @submit="onSubmit">
        <b-container>
          <h3>{{ $t('search-facilities.title1')}}</h3>
          <hr />
          <h5>{{ $t('search-facilities.title2')}}</h5>
          <b-row style="align:right;">
            <b-col>
              <b-form-group  :label="$t('search-facilities.category')"  >
                <b-form-select v-model="form.category" class="mb-3 select"  style="height: 42px !important;" required>
                  <option
                    v-for="category in categories"
                    v-bind:value="category.type"
                    v-bind:key="category.type"
                  >{{ $t('facilities' +"."+category.type) }}</option>
                </b-form-select>
              </b-form-group>
            </b-col>
            <b-col>
...
<!--b-col v-if="showlocation==''"><button @click="detectLocation"><img src="../assets/detect.png" height="20px" /></button></b-col-->
            <b-col>
              <b-form-group :label="$t('search-facilities.language')"  >
                <!--b-form-select v-model="form.language" :options="languages" class="mb-3 select"></b-form-select-->
          ...
          <input type="checkbox" :id="index" :value="option" v-model="languagepre">
           <label :for="index">{{ option.name }}</label>
            ...
            <div class="kov-search">
              <b-button class="select kov-lg" type="submit" style="height: 42px !important;margin-top:-1px;">
                <img src="../assets/search.png" height="14px" />
              </b-button>
            </div>
          </b-row>
        </b-container>
...
locationcountry: null,
  ...
  created() {
    ...
    if (self.category) {
      self.form.category = self.category;
    }
    this.isLoading = true;
    ...
    axios
      ...
      .then(function(response) {
        self.categories = response.data.searchcategories;
              ...
              for (let comp of locationComponents) {
                for (let type of comp.types) {
                 // Before this is afunction-->  if (!self.cityName && type === "administrative_area_level_2")
                  if (!self.cityName && type === "administrative_area_level_2") {
                  ...
                  if (!self.countryName && type === "country") {
                    ...
                    console.log('vikas',comp.long_name);
                    break;
                  }
                }
...
self.locationerror='';
      ...
      self.$router.push({ name: "SearchFacilities", query: this.form });  
